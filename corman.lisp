...
(defmethod emacs-inspect ((class standard-class))
          `("Name: " (:value ,(class-name class))
            (:newline)
            "Super classes: "
            ,@(comma-separated (c2mop:class-direct-superclasses class))
            (:newline)
            "Direct Slots: "
            ,@(comma-separated
               (c2mop:class-direct-slots class)
               (lambda (slot)
                 `(:value ,slot ,(princ-to-string (c2mop:slot-definition-name slot)))))
            (:newline)
            "Effective Slots: "
            ,@(if (c2mop:class-finalized-p class)
                  (comma-separated
            ...
            "Sub classes: "
            ,@(comma-separated (c2mop:class-direct-subclasses class)
                               (lambda (sub)
                                 `(:value ,sub ,(princ-to-string (class-name sub)))))
            (:newline)
            "Precedence List: "
            ,@(if (c2mop:class-finalized-p class)
                  (comma-separated (c2mop:class-precedence-list class)
                                         (lambda (class)
...
(defmethod emacs-inspect ((slot cons))
  ...
  (if (eq (car slot) :name)
              `("Name: " (:value ,(c2mop:slot-definition-name slot))
                         (:newline)
                         ,@(when (c2mop:slot-definition-documentation slot)
                             `("Documentation:"  (:newline)
                                                 (:value ,(c2mop:slot-definition-documentation slot))
                                                 (:newline)))
                         "Init args: " (:value ,(c2mop:slot-definition-initargs slot)) (:newline)
                         "Init form: "  ,(if (c2mop:slot-definition-initfunction slot)
                                             `(:value ,(c2mop:slot-definition-initform slot))
                                             "#<unspecified>") (:newline)
                                             "Init function: " (:value ,(c2mop:slot-definition-initfunction slot))
                                             (:newline))
      (call-next-method)))
